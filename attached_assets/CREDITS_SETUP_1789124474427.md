...
free. It is scoped to plans with **no online price**, which is exactly the set
that can never produce a webhook — so a subscriber can never collect twice.

No cron, and a workspace that never opens the app is never granted credits it
was not going to spend.
