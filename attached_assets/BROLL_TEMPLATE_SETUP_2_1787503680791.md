...
Five commits in `kokao-broll-composite.patch`.
...
git am kokao-broll-composite.patch              # applies as five commits
pnpm --filter @workspace/db run push            # commit 3 adds columns

# then, from artifacts/api-server:
pnpm exec tsx scripts/seed-video-templates.ts   # the four curated templates
pnpm exec tsx scripts/seed-prompt-kit.ts        # the beat-planner prompt
```
...
## Commit 5 — the templates themselves

`artifacts/api-server/src/lib/videoGen/videoTemplateSeeds.ts` plus
`scripts/seed-video-templates.ts`.

Four formats, published and pickable once seeded:

| Template | Length | Shape |
|---|---|---|
| Expert Explainer | 90s | Problem hook, technique named by 15s, two-thirds on demonstration, bare frame for the close |
| Quick Tip | 35s | One idea, four beats, no overlay for the first few seconds |
| Myth vs Fact | 60s | Myth in lifestyle footage, correction in graphics, bare frame on the correction |
| Product Walkthrough | 60s | Object in hand from second one, steps the viewer repeats |

Expert Explainer's numbers are the ones measured off the reference video. The
other three are variations on the same mechanics.

Seeding is idempotent by name, so re-running after an admin edit leaves the
edited row alone. Every row passes `assertTemplateSafe` before it is written and
again in CI. All four cost one video unit — none generate AI shots — so picking
one is never a cost surprise.

`kokao-video-templates.json` ships the same four rows as plain data if you would
rather insert them another way.
