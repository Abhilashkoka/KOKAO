...
Three commits in `kokao-broll-composite.patch`.
...
git am kokao-broll-composite.patch              # applies as three commits
...
The prompt ships as `BEAT_PLANNER_TEMPLATE_BLOCKS`, already in the shape
`prompt_template_versions` stores. New flow key: `video_broll_beats`.

1. Create a `prompt_case_types` row with `flowKey = 'video_broll_beats'`.
2. Create a `prompt_templates` row pointing at it.
3. Insert `BEAT_PLANNER_TEMPLATE_BLOCKS` as version 1 and point
   `activeProductionVersionId` at it.

After that, changing how beats get chosen is a content edit in the admin UI with
...
- 423 tests pass across 31 files in `videoGen` + the existing `videoStyles`
  route tests. 72 of those are new across the three commits.
- `redocly lint` — clean
- codegen drift — clean
