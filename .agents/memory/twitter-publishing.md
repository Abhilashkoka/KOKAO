...
- Token refresh on demand: `ensureFreshTwitterToken(tenantId, app)` returns `{ok, accessToken, accountName} | {ok:false, reason:"not_connected"|"reconnect_required"|"transient", message}`. Refreshes ~60s early via Basic-auth (`refreshTwitterTokens`, which throws a classified `TwitterRefreshError`) and persists the new token; never throws itself. Follows the shared refresh contract: only a DEFINITIVE rejection (invalid_grant/400/401) marks the row reconnect-needed; a transient outage (network/429/5xx) returns `reason:"transient"` and leaves the row untouched — `reverifyTwitter` just resets the check clock, publish paths surface a 503.
- Proactive on-load re-verify: `GET /twitter/status` calls `reverifyTwitter` (in `socialReverify.ts`, mirroring `reverifyFacebook`/`reverifyInstagram`) behind the shared `REVERIFY_STALE_MS` gate. It resolves a token via `ensureFreshTwitterToken`, then live-tests it with `testTwitterCredentials` (a `/2/users/me` read: 401/403 or missing user = dead/`failed`; 429/5xx/network = transient, only touch the clock). A verified→failed transition fires `notifySocialConnectionFailed` (platform label "X account"). Failed → card shows `expired:true` "Reconnect needed". This is why the per-tenant token IS live-testable even though the app-level confidential-client creds are not.
...
**App-level creds have no live test:** the confidential-client `clientId`/`clientSecret` cannot be validated without a full user authorization — presence of the row is the only "configured" signal. (This is separate from the per-tenant token, which IS live-testable via `/2/users/me`; see the proactive on-load re-verify bullet above.)
