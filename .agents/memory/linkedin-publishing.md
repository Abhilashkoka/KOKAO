...
- Posts API: `POST https://api.linkedin.com/rest/posts` with headers `LinkedIn-Version` + `X-Restli-Protocol-Version: 2.0.0`. LinkedIn RETIRES versions after ~1 year — a stale pinned version fails every publish with "Requested version X is not active" (502). Keep `LINKEDIN_VERSION` recent (env-overridable via `LINKEDIN_API_VERSION`) and bump it when this error appears. New post id is returned in the `x-restli-id` response header (status 201, no JSON body). Permalink: `https://www.linkedin.com/feed/update/{urn}`.
