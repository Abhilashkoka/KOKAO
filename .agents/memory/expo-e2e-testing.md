...
- Reusable bearer-token harness: `artifacts/api-server/scripts/mobile-signup-credits-e2e.mjs` scripts the full pattern (Clerk user→session→token, bearer request, DB asserts, cleanup) — copy it for other mobile-contract checks.
