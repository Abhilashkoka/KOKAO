...
---
Rule: in e2e test plans that seed tenant-scoped rows, resolve the tenant by polling `SELECT id FROM tenants WHERE email = <login_email>` (reload the app between retries) and NEVER fall back to "most recent tenant".
