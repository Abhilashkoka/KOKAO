...
- For an uncatalogued video model, activation checks whether the exact provider/model has at least one usable price row; it must not ask the runtime resolver to price an empty variant. Curated models still require every supported variant, and actual generation still requires an exact variant match. **Why:** conditional prices are valid configuration but deliberately cannot resolve `{}`, so using the runtime gate falsely rejects correctly priced models.
- An exact saved provider/model price wins before cross-catalog lookup. Cross-sourced marketplace pricing is only provisional when no exact row exists; never overwrite or warn over an admin-maintained exact rate.
