...
- Tap-clears-alert flow validated e2e (no device needed): real Clerk bearer + live API + Expo push mock (mock log now records full message payloads) — push data carried notificationId/badge, mark-read landed, unread count dropped. Simulate the tap by replaying the handler's exact API calls. Settings pushes (team/seat/billing, url "/settings") now deep-link to the Settings screen, so their taps DO mark the alert read (re-verified with the same harness); only taps that land ON the notifications feed keep their unread state.
