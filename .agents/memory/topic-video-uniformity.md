...
**Why:** the "or the story" prompt clause let characters change costume mid-video with no user request, independently-planned b-roll prompts drifted in palette/grade into a stock-collage look, and a shared-look direction alone can make distinct scenes converge on the same generic composition.
**How to apply:** when editing scene-director or art-director prompts/parsers in `lib/videoGen/topicVideo/`, keep the parser clamp, never replace style-append with image anchoring for b-roll, and preserve the explicit adjacent-shot diversity rule.
