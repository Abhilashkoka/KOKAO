...
- Threads (by Meta): `routes/threads.ts` — real OAuth via threads.net/oauth/authorize (no PKCE; state via shared `lib/oauthState.ts`), short→long-lived (`th_exchange_token`, 60 days) token, auto-refresh (`th_refresh_token`) in the status endpoint when <7 days remain. App creds are a SEPARATE Threads App ID/Secret inside the Meta app (NOT the Facebook App ID/Secret), stored superadmin-only in `app_credentials` provider "threads" with no env fallback. Publish chunks captions at 500 chars into a reply-chained thread; image goes on the first post via signed storage URL.
