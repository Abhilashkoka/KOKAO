...
An approved shared backdrop must canonically cover every script scene exactly once. Preview generation must load the approved backdrop bytes into the same compact reference sheet as the approved character/outfit bytes, and must require a reference-capable provider; prompt-only fallback is forbidden.
...
**How to apply:** Backdrop replacement invalidates every scene preview. Enqueue, preview, correction, retry, and final-render gates all revalidate full backdrop coverage and fingerprint integrity before reservation or provider dispatch.