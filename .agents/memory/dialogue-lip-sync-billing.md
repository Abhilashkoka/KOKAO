...
Treat an AI dialogue lip-sync render as two independent provider units: AI visual generation and lip-sync. A successful job must account for both actual provider events. Once either provider succeeds, later local processing or provider failures must preserve and settle that completed work rather than issuing a full refund. Per-second lip-sync models must be priced from the measured raw provider output, not narration/input duration.
