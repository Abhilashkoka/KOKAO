...
Save auto-tests immediately and persists `verifyStatus`/`verifyError`. The Facebook test checks BOTH read access (Page fetch) and publish permission: it inspects the token via Graph `/debug_token` (POST body, app access token `appId|appSecret` in the Authorization header) and rejects tokens missing `pages_read_engagement`/`pages_manage_posts` — a read-only token used to pass save then fail at publish with Graph error #200. The scope check is best-effort: it never blocks the save when app creds are absent or the debug call itself fails. Gotcha: a USER token with all the right scopes still can't publish to a Page (reads succeed, publish fails #200) — verification checks `debug_token` `type`, and for non-PAGE tokens auto-exchanges via `GET /<pageId>?fields=access_token` and persists the corrected Page token (save, retest, AND `reverifyFacebook` — the forced pre-publish reverify must persist it too or publish still uses the old token). Responses are ALWAYS masked (`maskSecret`) — plaintext secrets never leave the server and are never logged.

**Short-lived-token trap:** the long-lived upgrade (`fb_exchange_token`) only works for tokens issued by the SAME Meta app as the configured app creds — a token minted under another app (e.g. Graph API Explorer's default app) can never be upgraded and dies in ~1-2h no matter how often it's re-pasted. Verification now hard-fails these instead of silently storing them: `/debug_token` `app_id` mismatch → reject; final token with `expires_at` within 7 days after a failed upgrade → reject (`expires_at` 0 = never; debug unavailable = accept, never fail under uncertainty).
