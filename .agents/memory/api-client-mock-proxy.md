...
**Enforced:** the check lives in `src/test/apiClientMockGuardCheck.ts` and runs from BOTH a guard test (`apiClientMockGuard.test.ts`) and vitest `globalSetup` (`src/test/globalSetup.ts`), so it executes even on selective runs (`vitest related`, path filters) that skip the guard test file — don't bypass it, use the helper.
