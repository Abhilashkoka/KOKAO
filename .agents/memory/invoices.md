...
- Invoice emails: `recordInvoice` fires a post-commit fire-and-forget `notifyInvoiceIssued` (owner-only email with base64 PDF attachment via SendGrid `attachments`; in-app row always) ONLY for freshly inserted invoices — dedupe/replay hits stay silent. Delivery still passes both email gates (notification policy + app-level pause), so emails are opt-in by default.
