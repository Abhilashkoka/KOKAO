...
- Plan limits / quota helpers: `artifacts/api-server/src/lib/plans.ts` (async DB-backed store: `plan_settings` table overrides `DEFAULT_PLANS`, 30s cache, falls back to defaults on DB read failure without caching the degraded result; `getPlanLimits` is async — await it), `lib/usage.ts`. Superadmin plan editor: `PUT /admin/plans/:planId` in `routes/admin.ts` (fixed plan ids free/pro/business; -1 = unlimited; audits as `plan_edit` with null target), UI card `PlansCard` in `pages/admin.tsx`.
