 ...
 * A missing row means "use the defaults" (in-app on, email on) — an opt-out
 * model so a user who never opens the app still gets breakage emails until they
 * explicitly turn them off. What a tenant is actually allowed to change is
 * bounded by the global policy (notification_policies): e.g. when a type's email
 * policy is "forced" or "off" the stored `email` value is ignored at resolution
 * time.
 */
