// ../../../../../tmp/generate-job-552-scene1.ts
...
function decryptSecret(payload) {
  ...
  const data2 = Buffer.from(dataB64, "base64");
  let lastError;
...
var ObjectStorageService = class {
  ...
  async deletePublicBrandObject(servedPath) {
    const match2 = servedPath.match(/^\/api\/storage\/public-objects\/(brand\/[^/]+)$/);
    if (!match2) return;
    const file = await this.searchPublicObject(match2[1]);
...
async function signObjectURL({
  ...
  if (!response2.ok) {
    throw new Error(
      `Failed to sign object URL, errorcode: ${response2.status}, make sure you're running on Replit`
    );
  }
  const { signed_url: signedURL } = await response2.json();
  return signedURL;
}
...
var prompt = "app testing. Scene 1: tight medium close-up, centered frontal eye contact, steady camera; silent source plate with the person visibly talking naturally from the first second and throughout, natural but unmistakable speech-like motion, clear varied mouth shapes, regular open-and-close lip motion, visibly open vowel shapes, closed consonant shapes, and a relaxed moving jaw; no audible dialogue; exactly one unobstructed front-facing face remains large in frame throughout\n\nGenerate a portrait (2:3) image.";
var response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
  ...
  body: JSON.stringify({
    ...
    messages: [{ role: "user", content: [
      { type: "image_url", image_url: { url: `data:${String(metadata.contentType || "image/png")};base64,${referenceBuffer.toString("base64")}` } },
...
if (!response.ok) throw new Error(`OpenRouter failed (${response.status}): ${JSON.stringify(data).slice(0, 1e3)}`);
...
if (typeof message?.content === "string" && message.content.startsWith("data:image/")) candidates.push(message.content);
var url = candidates.find((value) => /^data:image\/[^;,]+;base64,/.test(value));
if (!url) throw new Error(`No image payload. message keys=${Object.keys(message || {}).join(",")} contentType=${typeof message?.content} images=${message?.images?.length ?? 0}`);
var match = /^data:image\/[^;,]+;base64,(.+)$/.exec(url);
await writeFile("/home/runner/workspace/attached_assets/generated_images/job-552-nano-banana-pro/scene-1.png", Buffer.from(match[1], "base64"));
console.log(`Generated Scene 1. message keys=${Object.keys(message || {}).join(",")}`);
