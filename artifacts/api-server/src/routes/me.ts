...
router.get("/me", async (req: Request, res: Response) => {
  ...
  const pendingInvite =
    req.memberRole === "owner"
      ...
      : null;
  const [legacyCredits, balance] = await Promise.all([
  ...
  res.json({
    ...
    // Keep this legacy shape for existing clients. The wallet's unified
    // balance is additive and intentionally lives under a separate key.
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    ...
    if (
      parsed.data.aiModel !== undefined &&
      !(await isAllowedTenantModel(parsed.data.aiModel))
    ) {
