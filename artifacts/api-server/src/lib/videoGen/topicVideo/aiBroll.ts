...
3. Each prompt describes ONE photorealistic, cinematic still that visualizes that scene's meaning: concrete subject, setting, and one slow camera move (e.g. glide, push-in, rise). Treat the full list as an edit plan, not independent illustrations. Consecutive prompts must differ in at least two of: main subject, setting, shot scale, camera angle/movement, and visual category (person, object/product, environment, graphic/data). Never reuse the same subject/activity, location, or composition in adjacent shots. When scenes make similar points, find a different visible consequence, object, or setting for the later scene rather than paraphrasing the previous shot. Vary the coverage across scenes — mix wide establishing shots, medium frames, intimate close-ups, details, and overheads where natural. Include the quality and direction of light (e.g. golden-hour warmth, diffused overcast, shafts of sunlight) and a tactile detail or atmosphere (surface texture, dust in sunlight, water reflections). Do not restate the style.
