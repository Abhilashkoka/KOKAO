...
export function GuidedStoryWorkflow({
  ...
  const [scriptRegenerationReason, setScriptRegenerationReason] = useState<string | null>(null);
      ...
      { draftId: sourceDraft.id, data: { revision: sourceDraft.revision } },
      {
        ...
        onError: (error) => {
          ...
          const nativeScriptInvalid =
            ...
            /Romanized|native (?:Hindi|Telugu|Tamil) script/i.test(message);
          if (!automaticRetry && nativeScriptInvalid) {
            setScriptRegenerationReason(
              "The first script used Romanized text instead of the selected language’s native script. KOKAO is regenerating it automatically.",
            );
            void queryClient
              .refetchQueries({
              ...
              .then(() => {
                const latest =
                  queryClient.getQueryData<GuidedStoryDraft>(
                    getGetGuidedStoryDraftQueryKey(sourceDraft.id),
                  ) ?? sourceDraft;
                requestScriptGeneration(latest, onSettled, true);
              })
              .catch(() => {
                setScriptRegenerationReason(null);
                setScriptGenerationError(
                  "The script used Romanized text and could not be regenerated automatically. Please try again.",
                );
                onSettled?.();
              });
            return;
...
function castApprovalInstruction(roles: Array<{ name: string }>) {
  ...
  if (names.length === 0) return "Approve the remaining cast role before starting final generation.";
  if (names.length === 1) return `Approve ${names[0]} before starting final generation.`;
  if (names.length === 2) return `Approve ${names[0]} and ${names[1]} before starting final generation.`;
  return `Approve ${names.slice(0, -1).join(", ")}, and ${names.at(-1)} before starting final generation.`;
}
...
function StoryFlow(props: any) {
      ...
      : props.existingJobId === null &&
        ...
        : props.existingJobId === null &&
          ...
          : props.existingJobId === null &&
            ...
            : !props.visualSaved
              ? "Save the visual consistency choices before starting final generation."
  ...
  const voiceLanguageNote = (
    <p className="text-sm text-muted-foreground" data-testid="text-guided-voice-language">
      Higgsfield receives the exact approved dialogue in the selected language, with one designated speaker per line and every intended character preserved in the scene.
    </p>
  );
  ...
  if (step === "ready") return <>
    {estimate}
    <p data-testid="status-guided-cast-complete">Cast is saved. Review and approve each role’s current references before compiling the final governed prompt.</p>
...
function LegacyBackdropReviewStep({ draft }: { draft: GuidedStoryDraft }) {
  ...
  if (!visuals) {
    return <Card data-testid="card-guided-backdrop-review"><CardHeader><CardTitle>Backdrop / location reference</CardTitle><CardDescription>Backdrop review is required before final generation.</CardDescription></CardHeader><CardContent><p className="text-sm text-destructive" role="alert" data-testid="error-guided-backdrop-missing">This older draft has no visual choices. Save visual consistency settings and prepare a backdrop reference before starting final generation.</p></CardContent></Card>;
  }
...
function VisualConsistencyStep(props: any) {
  ...
  return <Card className="border-primary/30" data-testid="guided-visual-consistency">
    ...
    <CardContent className="space-y-4">
      ...
      {!props.visualSaved && <p className="text-sm text-amber-700" data-testid="status-guided-visuals-unsaved">Save these choices before starting final generation.</p>}
