        ...
        return <Card key={role.id} className="border-primary/20" data-testid={`card-guided-cast-approval-${role.id}`}>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button type="button" onClick={() => props.onApproveCastRole(role.id)} disabled={props.pending || !cast} data-testid={`button-guided-approve-cast-${role.id}`}>{props.approvingCastRoleId === role.id ? "Approving…" : approved ? "Reapprove" : "Approve"}</Button>
            </div>
          </CardContent>
        </Card>;
      })}
