 ...
 * so a brief network blip doesn't permanently lose page views / feature
...
describe("web analytics requeue on failed send", () => {
  it("re-queues the batch when fetch throws (network blip)", async () => {
    ...
    expect(queue.map((e) => e.name)).toEqual(["page_view", "feature_use"]);
    ...
    expect(events.map((e) => e.name)).toEqual(["page_view", "feature_use"]);
