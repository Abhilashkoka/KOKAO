...
## Replit environment boundary

The SQL and backfill commands below are for the **development database** in this
workspace. Do not add them to application startup or the publish build command.
Replit-managed production schema changes go through Publish; that schema diff
does not run this recipient-data backfill.

If production contains legacy grants, do not approve removal of its legacy
recipient columns until their recipient records have been safely accounted for.
Completing the development backfill does not migrate production records.
